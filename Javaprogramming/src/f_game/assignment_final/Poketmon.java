...
public class Poketmon {
	...
	public void showPoketmonInfo(Poketmon p) {
		System.out.println("=========================================================");
	...
	void attack(Character ch) {
		int damage = att - ch.def;
		damage = damage <= 0 ? 1 : damage;
		ch.hp -= damage <= ch.hp ? damage : ch.hp;
		System.out.println("   　                 n  Λ＿Λ \n"
						 + "\t(ﾖ(´･ω･｀)  \n"
						 + "      ≡　y　 と丿 \n"
						 + "      ≡（_ﾉ　ノ  \n"
						 + "       ≡(ノ´");
		System.out.println(name + "(이)가 " + skill + "으로 " + ch.name1 + "에게 " + damage + "만큼 데미지를 주었습니다.");
		System.out.println(ch.name1 + "의 현재 체력: " + ch.hp + "/" + ch.maxHp);
	}
...
class Hoopa extends Poketmon{
	Hoopa() {
		super("후파", "6개의 링과 6개의 거대한 팔로 수많은 것들을 빼앗는다고 전해진다. 힘이 봉인되어 작은 모습으로 변했다.", 80, 110, 80, 1, 10, "다른차원홀", "에스퍼/고스트");
	}
}
